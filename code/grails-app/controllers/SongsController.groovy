    ...
    def list = {
        ...
        if (userid) {
            try {
                ...
                String sqlString = "select ProductID, ProductTitle, AuthorTitle, AlbumTitle, GenresTitle from TblProducts where ProductType='song'";
                if (title) {
                ...
                if (album) {
                    sqlString = sqlString + "  and AlbumTitle like '%" + album + "%'"
                }
                if (genres) {
                ...
                db.eachRow(sqlString) {
                    ...
                    retailerObj.each { row1 ->
                        stores << [credit: selectedUser.currentcredit, retailerid: row1.retailerid.id, retailertitle: row1.retailerid.retailertitle, retailerprice: row1.price]
                    }

                    data << [id: it.ProductID, title: it.ProductTitle, artist: it.AuthorTitle, album: it.AlbumTitle, genres: it.GenresTitle, purchased: recommendInstance ? 1 : 0, recommended: recommendInstance?.recommended ? 1 : 0, credit: selectedUser.currentcredit, store: stores]
                }
                if (data.size() > 0) {
                    responseData = [
                            'status' : 0,
